...
source "amazon-ebs" "base" {
  ami_name                    = "global-workflow-cluster-${local.now}.x86_64-gp3"
  ...
  tags                                  = { Name = "Global-Workflow-Cluster-${local.now}" }
...
build {
  source "amazon-ebs.base" {
    ami_description = "Global-Workflow Cluster"
    name            = "Global-Workflow-Cluster-Ubuntu-22.04-hvm"
    source_ami_filter {
      filters = {
